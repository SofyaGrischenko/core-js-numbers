...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const stepOne = x1 * x2 + y1 * y2;
  const stepTwo = Math.sqrt(x1 ** 2 + y1 ** 2);
  const stepTree = Math.sqrt(x2 ** 2 + y2 ** 2);
  const stepFour = stepOne / (stepTwo * stepTree);
  return Math.acos(stepFour);
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 3, fib = 1, prev = 1; i <= index; i += 1) {
    next = fib + prev;
    prev = fib;
    fib = next;
  }
...
function getCountOfOddNumbers(number) {
  ...
  return num % 2 === 0 ? num / 2 : Math.floor(num + 1) / 2;
}
